        ...
        if self.skills:
            if self.msg_type == 'SM':
                for sp in self.allocations:
                    mail_out = {}
                    mail_out['type'] = self.msg_type
                    if sp in self.allocations.keys():
...
class FmcSP(SP):
    ...
    def agent_receive_a_single_msg(self, msg):
        mail_in = msg.information
        self.msg_typs[msg.sender] = mail_in['type']

        if 'commit' in mail_in.keys():
            self.commits[msg.sender] =mail_in['commit']
    ...
    def calculate_start_and_end_times(self):
        ...
        if not self.old_schedule:
            ...
            print('lemgth'," : ", len(self.old_final_schedule)," : ",len(self.final_schedule) ," : ", len(self.old_final_schedule + self.final_schedule))
    ...
    def compute_R(self):  # fl     חישוב הR
        ...
        for sr in self.offers.keys():
            if self.msg_typs[sr] == 'HSM' or self.type == 'commit':
